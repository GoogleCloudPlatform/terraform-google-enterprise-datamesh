...
locals {
  ...
  is_cloud_build_v2        = var.tf_repo_type == "CLOUDBUILDv2"
  cloudbuild_repo_id_split = local.is_cloud_build_v2 ? split("/", var.tf_repo_uri) : []
  cloud_build_repo_name    = local.is_cloud_build_v2 ? local.cloudbuild_repo_id_split[length(local.cloudbuild_repo_id_split) - 1] : ""
  ...
  repo           = local.is_source_repo ? local.source_repo_name : local.is_cloud_build_v2 ? local.cloud_build_repo_name : local.gh_name
...
resource "google_cloudbuild_trigger" "triggers" {
  ...
  dynamic "repository_event_config" {
    for_each = local.is_cloud_build_v2 ? [1] : []
    content {
      repository = var.cloudbuildv2_repo_id
