...
locals {
  suffix = var.add_random_suffix ? random_id.bucket_suffix[0].hex : ""
}
...
resource "google_storage_bucket" "bucket" {
  ...
  name     = join("-", [var.gcs_bucket_prefix, var.environment_code, var.name, local.suffix])
  ...
  encryption {
    default_kms_key_name = data.google_kms_crypto_key.reidentification_key.id
  }
