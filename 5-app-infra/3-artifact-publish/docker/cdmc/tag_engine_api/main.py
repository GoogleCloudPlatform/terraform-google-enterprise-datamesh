...
TAG_ENGINE_SA = os.getenv('TAG_ENGINE_SA', config['DEFAULT']['TAG_ENGINE_SA'].strip())
TAG_CREATOR_SA = os.getenv('TAG_CREATOR_SA', config['DEFAULT']['TAG_CREATOR_SA'].strip())

TAG_ENGINE_PROJECT = os.getenv('TAG_ENGINE_PROJECT', config['DEFAULT']['TAG_ENGINE_PROJECT'].strip())
TAG_ENGINE_REGION = os.getenv('TAG_ENGINE_REGION', config['DEFAULT']['TAG_ENGINE_REGION'].strip())

FIRESTORE_PROJECT = os.getenv('FIRESTORE_PROJECT', config['DEFAULT'].get('FIRESTORE_PROJECT').strip())
FIRESTORE_DB = os.getenv('FIRESTORE_DB', config['DEFAULT'].get('FIRESTORE_DB', '(default)').strip())

BIGQUERY_REGION = os.getenv('BIGQUERY_REGION', config['DEFAULT'].get('BIGQUERY_REGION', '').strip() or None)
SPANNER_REGION = os.getenv('SPANNER_REGION', config['DEFAULT'].get('SPANNER_REGION', '').strip() or None)
FILESET_REGION = os.getenv('FILESET_REGION', config['DEFAULT'].get('FILESET_REGION', '').strip() or None)

SPLIT_WORK_HANDLER = os.getenv('SERVICE_URL', '') + '/_split_work'
RUN_TASK_HANDLER = os.getenv('SERVICE_URL', '') + '/_run_task'

INJECTOR_QUEUE = os.getenv('INJECTOR_QUEUE', config['DEFAULT']['INJECTOR_QUEUE'].strip())
WORK_QUEUE = os.getenv('WORK_QUEUE', config['DEFAULT']['WORK_QUEUE'].strip())
...
ENABLE_TAG_HISTORY = os.getenv('ENABLE_TAG_HISTORY', config['DEFAULT'].get('ENABLE_TAG_HISTORY', False)).strip().lower() == 'true'

TAG_HISTORY_PROJECT = os.getenv('TAG_HISTORY_PROJECT', config['DEFAULT'].get('TAG_HISTORY_PROJECT', '').strip() or None)
TAG_HISTORY_DATASET = os.getenv('TAG_HISTORY_DATASET', config['DEFAULT'].get('TAG_HISTORY_DATASET', '').strip() or None)

ENABLE_JOB_METADATA = os.getenv('ENABLE_JOB_METADATA', config['DEFAULT'].get('ENABLE_JOB_METADATA', False)).strip().lower() == 'true'

JOB_METADATA_PROJECT = os.getenv('JOB_METADATA_PROJECT', config['DEFAULT'].get('JOB_METADATA_PROJECT', '').strip() or None)
JOB_METADATA_DATASET = os.getenv('JOB_METADATA_DATASET', config['DEFAULT'].get('JOB_METADATA_DATASET', '').strip() or None)


print(f"TAG_ENGINE_SA: {TAG_ENGINE_SA}")
print(f"TAG_CREATOR_SA: {TAG_CREATOR_SA}")

print(f"TAG_ENGINE_PROJECT: {TAG_ENGINE_PROJECT}")
print(f"TAG_ENGINE_REGION: {TAG_ENGINE_REGION}")

print(f"FIRESTORE_PROJECT: {FIRESTORE_PROJECT}")
print(f"FIRESTORE_DB: {FIRESTORE_DB}")

print(f"BIGQUERY_REGION: {BIGQUERY_REGION}")
print(f"SPANNER_REGION: {SPANNER_REGION}")
print(f"FILESET_REGION: {FILESET_REGION}")

print(f"SPLIT_WORK_HANDLER: {SPLIT_WORK_HANDLER}")
print(f"RUN_TASK_HANDLER: {RUN_TASK_HANDLER}")

print(f"INJECTOR_QUEUE: {INJECTOR_QUEUE}")
print(f"WORK_QUEUE: {WORK_QUEUE}")

print(f"SCOPES: {SCOPES}")
print(f"USER_AGENT: {USER_AGENT}")

print(f"ENABLE_TAG_HISTORY: {ENABLE_TAG_HISTORY}")
print(f"TAG_HISTORY_PROJECT: {TAG_HISTORY_PROJECT}")
print(f"TAG_HISTORY_DATASET: {TAG_HISTORY_DATASET}")

print(f"ENABLE_JOB_METADATA: {ENABLE_JOB_METADATA}")
print(f"JOB_METADATA_PROJECT: {JOB_METADATA_PROJECT}")
print(f"JOB_METADATA_DATASET: {JOB_METADATA_DATASET}")
...
# Enable Auth: check if env variable is set, otherwise check ini file
ENABLE_AUTH = os.getenv('ENABLE_AUTH', config['DEFAULT'].get('ENABLE_AUTH', 'false')).strip().lower() in ['true', '1']
...
# OAuth Client Credentials: check if env variable is set, otherwise check ini file
OAUTH_CLIENT_CREDENTIALS = os.getenv('OAUTH_CLIENT_CREDENTIALS', config['DEFAULT'].get('OAUTH_CLIENT_CREDENTIALS', '').strip())

if OAUTH_CLIENT_CREDENTIALS:
    print('Info: OAUTH_CLIENT_CREDENTIALS =', OAUTH_CLIENT_CREDENTIALS)
else:
    if 'tag-engine-ui-' in os.getenv('SERVICE_URL', ''):
