...
class TagEngineStoreHandler:
    ...
    def __init__(self):
        ...
        # Get FIRESTORE_PROJECT from environment or config
        self.firestore_project = os.getenv(
            'FIRESTORE_PROJECT', 
            config['DEFAULT'].get('FIRESTORE_PROJECT', config['DEFAULT']['TAG_ENGINE_PROJECT']).strip()
        )

        # Get FIRESTORE_DB from environment or use default
        self.firestore_db = os.getenv(
            'FIRESTORE_DB', 
            config['DEFAULT'].get('FIRESTORE_DB', '(default)').strip()
        )
        
        self.db = firestore.Client(
            project=self.firestore_project, 
            database=self.firestore_db, 
