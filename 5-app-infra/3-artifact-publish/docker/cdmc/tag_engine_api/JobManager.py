...
if __name__ == '__main__':
    ...
    queue_name = os.getenv('INJECTOR_QUEUE', config['DEFAULT']['INJECTOR_QUEUE'].strip())
    task_handler_uri = '/_split_work'
    db_project = FIRESTORE_PROJECT = os.getenv('FIRESTORE_PROJECT', config['DEFAULT'].get('FIRESTORE_PROJECT').strip())
    db_name = os.getenv('FIRESTORE_DB', config['DEFAULT'].get('FIRESTORE_DB', '(default)').strip())
