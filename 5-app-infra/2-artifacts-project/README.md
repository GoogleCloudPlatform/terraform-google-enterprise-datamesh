...
## Deploying gcp-data-prg-domain-1-artifacts

1. clone your repository that was created in 4-projects
   ```bash
   git clone git@github.com:[git-owner-name]/gcp-dm-bu4-prj-artifacts.git bu4-prj-artifacts
   ```

1. cd over to the `bu4-prj-artifacts` directory
    ```bash
    cd bu4-prj-artifacts
    ```
...
1. Copy contents to new repo.
   ```bash
   cp -R ../gcp-data-mesh-foundations/5-app-infra/2-artifacts-project/* .
   ```

1. Update the `backend.tf` files with the backend bucket from step Projects.
    ```bash
    export backend_bucket=$(terraform -chdir="../gcp-projects/business_unit_4/shared" output  -json state_buckets | jq -r '."artifacts"')
...
1. Update `remote_state_bucket` in common.auto.tfvars
   ...
   sed -i'' -e "s/REMOTE_STATE_BUCKET/${remote_state_bucket}/" shared.auto.tfvars
   ```
